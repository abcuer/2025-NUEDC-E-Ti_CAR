#include "headfile.h"
unsigned short Anolog[8]={0};//Temp
unsigned short White[8]={0};
unsigned short Black[8]={0};
No_MCU_Sensor NNSENSOR;
/**
*@params No_MCU_SensorPtr
*@return 0: NOT Finished,,,1:Finished
*/
uint8_t Calibrate_Sensor(No_MCU_Sensor* NNSENSOR)
{
	  uint8_t sta=0;
		No_MCU_Ganv_Sensor_Init_Frist(NNSENSOR);
		No_Mcu_Ganv_Sensor_Task_Without_tick(NNSENSOR);
		Get_Anolog_Value(NNSENSOR,Anolog);//White
	  memcpy(Anolog,White,sizeof(Anolog));
    
		Get_Anolog_Value(NNSENSOR,Anolog);//Black
	  memcpy(Anolog,Black,sizeof(Anolog));
	  memset(Anolog,0,sizeof(Anolog));
	  
	  delay_ms(100);
	//得到黑白校准值之后，初始化传感器
		No_MCU_Ganv_Sensor_Init(NNSENSOR,White,Black);
	  sta = 1;
	return sta;
}
